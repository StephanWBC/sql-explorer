...
- `ConnectionDialogViewModel` handles all auth flows and saved connection management
...
- Saved connections stored as JSON in `~/.sqlexplorer/connections.json`, settings in `~/.sqlexplorer/settings.json`

## CI/CD

GitHub Actions (`.github/workflows/release.yml`) triggers on:
- **Every push to main** — builds macOS DMG + Windows ZIP as artifacts
- **Tags matching `v*`** — builds + creates a GitHub Release with installers attached
